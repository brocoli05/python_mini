...
def reset_timer():
    ...
    reps = 0
    checkmark.config(text="")  # reset checkmarks
...
def count_down(count):
    ...
    if count_sec < 10:
        count_sec = f"0{count_sec}"  # change data type into str

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)  # 1000: 1 sec
    else:
        start_timer()
...
# highlightthickness=0 : erase the outline
